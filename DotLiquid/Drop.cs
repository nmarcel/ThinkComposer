...
namespace DotLiquid
{
    ...
    internal class TypeResolution
    {
        ...
        private Dictionary<string, T> GetMemberDictionary<T>(IEnumerable<T> members,
            ...
            where T : MemberInfo
        {
            //$ Changed to detect duplicate-key exception
            Dictionary<string, T> rr = null;

            try
            {
                // Remove duplicates
                members = Instrumind.Common.General.RemoveDuplicateSurplus(members, item => item.Name);

                rr = members.Where(filterMemberCallback).ToDictionary(mi =>
                    Template.NamingConvention.GetMemberName(mi.Name),
                    Template.NamingConvention.StringComparer);
            }
            catch(Exception Problem)
            {
                Console.WriteLine("Exception: " + Problem.Message);
            }
    ...
    public abstract class DropBase : ILiquidizable, IIndexable, IContextAware
    {
        ...
        //$ made public to be accesible in ThinkComposer
    ...
    public class DropProxy : DropBase, IValueTypeConvertible
    {
        ...
        //$ Extensions to support dynamic objects (gets all the properties)
        public override object BeforeMethod(string method)
        {
            return ExtractValue(this._proxiedObject, method);
        }

        public static object ExtractValue(object source, string propName)
        {
            if (source == null || String.IsNullOrWhiteSpace(propName))
                return null;

            object result = null;

            if (source is DynamicObject)
                result = Instrumind.Common.General.ExtractFieldValue(source, propName).Item2;

            if (result == null && source is IIndexable)
                result = ((IIndexable)source)[propName];

            if (result == null && source is Instrumind.Common.EntityBase.IMModelClass)
            {
                var instance = (Instrumind.Common.EntityBase.IMModelClass)source;
                var propDef = instance.ClassDefinition.GetPropertyDef(propName, false);
                if (propDef != null)
                    result = propDef.Read(instance);
            }

            if (result == null)
                result = Instrumind.Common.General.ExtractPropertyValue(source, propName, false).Item2;
